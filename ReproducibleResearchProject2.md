...
Download the raw data file and extract the data into a dataframe.Then convert to a data.tab
...
DataStorm <- read.csv("/Users/mgalarny/Desktop/repdata%2Fdata%2FStormData.csv.bz2")

# Converting data.frame to data.tab
stormDT <- as.data.tab(DataStorm)
```
...
Melting data.tab so that it is easier to put in bar graph format
...
``` r
# Create CHARTs
healthCHARTs <- ggplot(bad_stuff, aes(x=reorder(EVTYPE, -value), y=value))
# Plot data as bar CHARTs
healthCHARTs = healthCHARTs + geom_bar(stat="identity", aes(fill=bad_thing), position="dodge")
# Format y-axis scale and set y-axis label
healthCHARTs = healthCHARTs + ylab("Frequency Count") 
# Set x-axis label
healthCHARTs = healthCHARTs + xlab("Event Type") 
# Rotat x-axis tick labels 
healthCHARTs = healthCHARTs + theme(axis.text.x = element_text(angle=45, hjust=1))
# Set CHARTs title and center it
healthCHARTs = healthCHARTs + ggtitle("Top 10 US Killers") + theme(plot.title = element_text(hjust = 0.5))
healthCHARTs
```

![](ReproducibleResearchProject2_files/figure-markdown_github/healthCHARTs-1.png)
...
``` r
# Create CHARTs
econCHARTs <- ggplot(econ_consequences, aes(x=reorder(EVTYPE, -value), y=value))
# Plot data as bar CHARTs
econCHARTs = econCHARTs + geom_bar(stat="identity", aes(fill=Damage_Type), position="dodge")

# Format y-axis scale and set y-axis label
econCHARTs = econCHARTs + ylab("Cost (dollars)") 

# Set x-axis label
econCHARTs = econCHARTs + xlab("Event Type") 

# Rotate x-axis tick labels 
econCHARTs = econCHARTs + theme(axis.text.x = element_text(angle=45, hjust=1))

# Set CHARTs title and center it
econCHARTs = econCHARTs + ggtitle("Top 10 US Storm Events causing Economic Consequences") + theme(plot.title = element_text(hjust = 0.5))
